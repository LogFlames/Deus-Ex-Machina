...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (!m_Jump && (m_CharacterController.isGrounded || (doubleJumpLeft && doubleJump)))
            {
        ...
        private void FixedUpdate()
        {
            ...
            if (m_CharacterController.isGrounded || (doubleJumpLeft && doubleJump))
            {
                if (m_Jump)
                {
                    if (!m_CharacterController.isGrounded)
        ...
        private void HandleDashStart()
        {
            ...
            if (dashCountdownTimer <= 0f)
            {
                if (Input.GetKeyDown(activateDashKey) && dashLeftThisJump)
                {
                    ...
                    m_MoveDir = dashDirection * (dashDistance / dashDuration);
                }
            }
        }
        ...
        private void HandleDash()
        {
            ...
            if (currentlyDashing && dashDurationCountdownTimer <= 0f)
            {
                ...
                m_MoveDir = velocityBeforeDash;

                m_MoveDir.y = Mathf.Max(m_MoveDir.y, 0f);
            }

            dashDurationCountdownTimer -= Time.fixedDeltaTime;
        }
